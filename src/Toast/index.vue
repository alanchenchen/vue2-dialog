...
export default {
	/**
	* comoponent: Toast 状态提示
	* author： Alan Chen
	* contact：739709491@qq.com
	* lastDate: 2018/10/15
	* 使用：
	*	props: 
	*  	1. value， 布尔值，控制toast显示与否,可以使用v-model绑定变量，默认为false
	*		2. data，对象，toast选项。包括：content，time和type等键值。
	*				* content,可选,字符串，html或者文字，显示在toast里的内容
	* 			* time,可选,数字,默认500ms,控制toast停留的时间，单位是毫秒
	*				* type,可选,字符串,默认default,控制toast显示的方式
	*							有middle,top，bottom，default,success,fail,warn,text，和loading状态
	* 						text为显示长文本，success,fail和warn表示成功、失败和警告三种状态
	*							loading有四种状态，分别是loading-circle，loading-ball，loading-wave和mini-loading(loading在左侧)
	*
	*	注意：
	*		1. toast组件为了只关注组件本身的逻辑，对于非loading状态，不需要处理value，会根据time的值自动倒计时来关闭toast
	*		2. 对于loading状态，则只需要开发者自行调用value绑定的变量为false来关闭
	*/
	name:'Toast',
	props: {
		value: {
	...
	methods: {
		hide() {
			const interval = this.data.time || 500 
			if(!this.data.type || this.data.type.indexOf('loading') < 0){
				setTimeout(() => {
